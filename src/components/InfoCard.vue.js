...
import { CodeBracketIcon, ShieldCheckIcon } from '@heroicons/vue/24/outline';
import IconWebHook from '@/assets/Icons/IconWebHook.vue';
export default defineComponent({
    name: 'InfoCard',
    data() {
        return {
            extendCss: this.$props.icon == 'Webhook' ? 'fill-emerald-300' : ''
        };
    },
    props: {
        icon: String,
...
function __VLS_template() {
    ...
    // @ts-ignore
    const __VLS_1 = __VLS_asFunctionalComponent(__VLS_0, new __VLS_0({ ...{ class: ((`h-8 w-8 text-emerald-300 ${__VLS_ctx.extendCss}`)) }, }));
    const __VLS_2 = __VLS_1({ ...{ class: ((`h-8 w-8 text-emerald-300 ${__VLS_ctx.extendCss}`)) }, }, ...__VLS_functionalComponentArgsRest(__VLS_1));
